...
pub async fn get_player_avg_stats(Path(id): Path<i64>, State(state): State<AppState>) -> Response {
    ...
    let data: NbaPlayerAvg = match serde_json::from_str(&output) {
        Ok(d) => d,
        Err(e) => {
            tracing::error!("Invalid JSON from Python: {e}");
            return (
...
pub async fn get_player_info(Path(id): Path<i64>, State(state): State<AppState>) -> Response {
    ...
    let data: NbaPlayerInfo = match serde_json::from_str(&output) {
        Ok(d) => d,
        Err(e) => {
            tracing::error!("Invalid JSON from Python: {e}");
            return (
...
pub async fn get_player_avg5(Path(id): Path<i64>, State(state): State<AppState>) -> Response {
    ...
    let data: NbaPlayerLast5 = match serde_json::from_str(&output) {
        Ok(d) => d,
        Err(e) => {
            tracing::error!("Invalid JSON from Python: {e}");
            return (
